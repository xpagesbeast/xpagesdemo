...
public class DeleteRowListener implements ActionListener{
	...
	public void processAction(ActionEvent event) throws AbortProcessingException {
		...
		*/
		if(null!=CommonUtil.getInstance().getBeanFromViewScope(ConstantsBean.CUSTOMER_BEAN)){
			System.out.println(btnName + " bean found.");
			if(opCode.equals("Customer")){
				((PageBean)CommonUtil.getInstance().getBeanFromViewScope(ConstantsBean.CUSTOMER_BEAN)).setDeleteCustomerItemRow(rowIndex);
			}else{
				//((PageBean)CommonUtil.getInstance().getBeanFromViewScope(ConstantsBean.CUSTOMER_BEAN)).loadCurrentCustomer(rowIndex,"edit");
			}
